...
export async function uploadLicenseFile(
  ...
  const commentText = `License key generated by ${uploadedByName} for request #${request.requestNo}. Valid: ${startDate} – ${endDate}.${fingerprintNote}`;
  await db.insert(onpremComments).values({
    deploymentId,
    comment: commentText,
